...
app.get('/api/match/status', (req, res) => {
  ...
  if (activeMatches[uid]) return res.json({ status:'matched', ...activeMatches[userId] });
...
app.post('/api/challenge/respond', (req, res) => {
  ...
  activeMatches[c.toUserId]={ matchId:mid, opponent:{id:c.fromUserId,username:from.username,score:from.score,wins:from.wins,losses:from.losses,draws:to.draws,recentGames:from.recentGames||[]}, color:ca, myOppPattern:c.pattern||'default' };
